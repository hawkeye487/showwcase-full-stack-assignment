...
const Main: React.FC = () => {
  ...
  const {
    ...
    },
    {
      ...
      initialData: queryClient.getQueryData<EducationData[]>('educations'), // Use initial data from the cache, if available
    }
  );
  ...
  const updateEducationMutation = useMutation(
    async (updatedEducation: EducationData) => {
      const token = await getToken();
      const response = await fetch(`${BASE_URL}/education/${updatedEducation.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  return (
    <MainContainer>
      <ContentContainer>
        <h2>Main Screen</h2>
        <AddButton onClick={() => setShowModal(true)}>Add Education</AddButton>
      </ContentContainer>
      <EducationWrapper>
